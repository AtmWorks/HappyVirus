...
public class FadeToBlack : MonoBehaviour
{
        ...
        if (cameraEffect == true && timer > 1.5)
        {
            ...
            else
            {
                if(newArea != null)
                {
                    ...
                    // Obt�n una referencia al componente ResetObjectsToDefault
                    
                    ResetObjectsToDefault resetComponent = newArea.GetComponent<ResetObjectsToDefault>();

                    // Verifica si el componente se encontr� antes de llamar al m�todo
                    if (resetComponent != null)
                    {
                        // Llama al m�todo ResetToDefault() en el componente ResetObjectsToDefault
    ...
    //{
    //    // Aseg�rate de que spawnControllerManager y currentTPs no sean nulos.
